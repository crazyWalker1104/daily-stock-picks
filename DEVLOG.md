...
**定位**：独立于每日推荐管线，专注 1 只股票的深度量化跟踪 + 买卖信号推送。
...
**不做的**：自动化交易执行(需QMT API)、多标的组合、ML模型、盘中实时tick、GitHub Actions自动推送
...
**待办事项：**
- [x] Phase 3.3: 策略分层 — 追强/抄底/事件驱动三条线 ✅
- [ ] Phase 3.4: 策略回测 — 按策略维度分组对比胜率
- [ ] 增加策略标签到数据库 stocks 表
...
**完成事项：**
- 🚀 **Phase 3.3 完成**：新增 [src/strategy_classifier.py](src/strategy_classifier.py) 策略分层模块
  - `StrategyClassifierEngine`: Engine + Singleton + Convenience 模式
  - 三维规则打分：追强(🚀) / 抄底(🎯) / 事件驱动(⚡)
  - 每条推荐注入 strategy/strategy_score/strategy_advice 动态属性
  - 管线 Phase 2.3 注入（技术面过滤后）、四格式输出
- 🐛 修复：`_safe_float()` 处理 push2 API 字符串数值
- 🔧 配置 + 文档同步更新
- 📝 CLI/Markdown/邮件/网页四格式策略标签显示
- 📝 更新 CLAUDE.md + DEVLOG.md

**今日管道：**
- 06-10: 61条新闻 → 4条推荐，策略：⚡事件驱动3 · 👀观望1
- push2 API 偶发断连 → 技术评分默认63，fallback 正常
- [ ] 雪球爬虫启用（配置 cookie）
...
**完成事项：**
- 🐛 **技术评分虚假63分修复** — 盘前 push2 全0数据检测
  - `_compute_score()`: 行情全0时跳过虚假加减分，仅用K线信号
  - `_filter_single_stock()`: 新增 `quote_missing` 标记
  - `get_summary()`: 盘前显示"⏳ 待盘中确认"而非虚假分数
  - 四格式输出：行情缺失时隐藏评分条，显示MA状态
- 🚀 **Phase 3.4 完成**：策略回测 — 按策略维度胜率对比
  - DB schema 新增 `strategy` / `strategy_score` 列
  - `save_report()` 持久化策略标签
  - `backfill_strategies()`: 从已有 technical_json 回填策略
  - `get_strategy_stats()`: 按追强/抄底/事件驱动分组统计
  - 新增 CLI: `--strategy` `--backfill-strategy`
  - `_classify_from_db()`: StrategyClassifier 新增 DB 数据适配方法
  - `_print_report()`: 报告详情显示策略标签
